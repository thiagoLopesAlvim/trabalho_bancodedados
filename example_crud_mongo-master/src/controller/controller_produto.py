...
class Controller_Produto:
    ...
    def inserir_produto(self) -> Produto:
        ...
        nome_novo_produto = input("Nome (Novo): ")
        ...
        id_produto = self.mongo.db["produtos"].insert_one({"codigo_produto": proximo_produto, "nome_produto": nome_novo_produto})
        # Recupera os dados do novo produto criado transformando em um DataFrame
        df_produto = self.recupera_produto(id_produto.inserted_id)
        # Cria um novo objeto Produto
        novo_produto = Produto(df_produto.codigo_produto.values[0], df_produto.nome_produto.values[0])
    ...
    def atualizar_produto(self) -> Produto:
        ...
        if not self.verifica_existencia_produto(codigo_produto):
            ...
            nova_nome_produto = input("Nome (Novo): ")
            # Atualiza a descrição do produto existente
            self.mongo.db["produtos"].update_one({"codigo_produto": codigo_produto}, {"$set": {"nome_produto": nova_nome_produto}})
            # Recupera os dados do novo produto criado transformando em um DataFrame
            df_produto = self.recupera_produto_codigo(codigo_produto)
            # Cria um novo objeto Produto
            produto_atualizado = Produto(df_produto.codigo_produto.values[0], df_produto.nome_produto.values[0])
            # Exibe os atributos do novo produto
    ...
    def excluir_produto(self):
        ...
        if not self.verifica_existencia_produto(codigo_produto):            
            ...
            produto_excluido = Produto(df_produto.codigo_produto.values[0], df_produto.nome_produto.values[0])
    ...
    def verifica_existencia_produto(self, codigo:int=None, external: bool = False) -> bool:
        ...
        # Recupera os dados do novo produto criado transformando em um DataFrame
        df_produto = pd.DataFrame(self.mongo.db["produtos"].find({"codigo_produto":codigo}, {"codigo_produto": 1, "nome_produto": 1, "_id": 0}))
    ...
    def recupera_produto(self, _id:ObjectId=None) -> pd.DataFrame:
        # Recupera os dados do novo produto criado transformando em um DataFrame
        df_produto = pd.DataFrame(list(self.mongo.db["produtos"].find({"_id":_id}, {"codigo_produto": 1, "nome_produto": 1, "_id": 0})))
        return df_produto
    ...
    def recupera_produto_codigo(self, codigo:int=None, external: bool = False) -> pd.DataFrame:
        ...
        # Recupera os dados do novo produto criado transformando em um DataFrame
        df_produto = pd.DataFrame(list(self.mongo.db["produtos"].find({"codigo_produto":codigo}, {"codigo_produto": 1, "nome_produto": 1, "_id": 0})))
