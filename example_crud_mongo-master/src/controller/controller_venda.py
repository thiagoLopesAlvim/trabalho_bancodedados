...
class Controller_Venda:
    ...
    def inserir_venda(self) -> Venda:
        ...
        id_venda = self.mongo.db["vendas"].insert_one({"codigo_venda": proximo_venda,"data_venda" : data_hoje ,"cpf": cpf, "cpfV": cpfV, "codigo_produto":codigoP})
    ...
    def recupera_venda(self, _id:ObjectId=None) -> pd.DataFrame:
        # Recupera os dados do novo pedido criado transformando em um DataFrame
    ...
    def recupera_venda_codigo(self, codigo:int=None, external: bool = False) -> bool:
        ...
        # Recupera os dados do novo pedido criado transformando em um DataFrame
        df_venda = pd.DataFrame(list(self.mongo.db["vendas"].find({"codigo_venda": codigo}, {"codigo_venda": 1, "data_venda": 1, "cpf": 1, "cpfV": 1,"codigo_produto":1, "_id": 0})))
